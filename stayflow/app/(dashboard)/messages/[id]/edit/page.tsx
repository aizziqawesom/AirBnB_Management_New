...
import { getTemplate, updateTemplate } from '@/lib/actions/messages';
import { TEMPLATE_VARIABLES, SAMPLE_DATA, type MessageRecipient, type MessageTemplate } from '@/lib/types/message';

interface EditMessageTemplatePageProps {
  params: Promise<{ id: string }>;
}

export default function EditMessageTemplatePage({ params }: EditMessageTemplatePageProps) {
  ...
  useEffect(() => {
    ...
    async function fetchTemplate() {
      ...
      const templateData = result.data as MessageTemplate;
      setTitle(templateData.title);
      setRecipient(templateData.recipient);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await updateTemplate(resolvedParams.id, {
        title,
        recipient,
        template,
      });
