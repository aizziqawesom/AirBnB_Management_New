...
export async function POST(request: NextRequest) {
  try {
    ...
    const sampleBookingData = {
      ...
      checkOutDate: 'Dec 25, 2024',
      bookingReference: 'BK123456',
      totalPrice: 'RM 2,500.00',
      numGuests: '4 guests',
      status: 'Confirmed',
    };
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: [email],
      subject: `Booking ${sampleBookingData.status} - ${sampleBookingData.propertyName}`,
      react: BookingEmailTemplate(sampleBookingData),
    });
